...
class InstallTracker:
    ...
    INSTALL_STEPS = [
        ...
        InstallStep("dep_scan", "Dependency Scan",
                    "Scanning system for existing dependencies to reuse", order=2),
        InstallStep("venv_setup", "Virtual Environment",
                    "Creating .venv virtual environment or linking existing", order=3),
        InstallStep("deps_install", "Core Dependencies",
                    "Installing requirements.txt packages (or linking)", order=4),
        InstallStep("gpu_detect", "Hardware Detection",
                    "Detecting NVIDIA GPUs and compute capability", order=5),
        InstallStep("sdk_validate", "SDK Validation",
                    "Validating SLATE SDK imports and version", order=6),
        InstallStep("dirs_create", "Directory Structure",
                    "Creating workspace directories and init files", order=7),
        InstallStep("git_sync", "Git Synchronization",
                    "Syncing with GitHub repository state", order=8),
        InstallStep("benchmark", "System Benchmark",
                    "Running CPU, memory, disk, and GPU benchmarks", order=9),
        InstallStep("runtime_check", "Runtime Verification",
                    "Final verification of all integrations", order=10),
    ]
